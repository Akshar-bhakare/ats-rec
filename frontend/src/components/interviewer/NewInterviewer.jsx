...
export default function NewInterviewer() {
    const [interviewerState] = useInterviewerContextState();
    const navigate = useNavigate();
    const theme = useTheme();

    const [initial,] = useState(interviewerState?.interviewerInitialValuesDict || {});
    const [alertConfig, setAlertConfig] = useState({ open: false, message: '', severity: 'info' });
    const [passwordError, setPasswordError] = useState('');
    const [showPassword, setShowPassword] = useState(false);
    const handleAlertClose = (_e, reason) => { if (reason !== 'clickaway') setAlertConfig(a => ({ ...a, open: false })); };

    const isEditing = Boolean(initial?._id);
    const formTitle = isEditing ? 'Edit Interviewer' : 'New Interviewer';
    const formSubtitle = isEditing ? 'Update interviewer details' : 'Enter interviewer details';

    const fieldPadding = useMemo(() => ({ px: { xs: 2, sm: 3 }, mb: 2 }), []);
    const firstFieldPadding = useMemo(() => ({ px: { xs: 2, sm: 3 }, pt: { xs: 2, sm: 3 }, mb: 2 }), []);
    ...
    }), [theme.palette.background.default, theme.palette.divider, theme.palette.primary.main]);
    ...
    const fields = useMemo(() => [
                    ...
                    InputProps={{
                        startAdornment: (
                            <InputAdornment position="start">
                                ...
                                >
                                    {countryList.map(c => (
                                        <MenuItem key={c.code} value={c.dial_code} sx={{ fontSize: '0.9rem', whiteSpace: 'nowrap' }}>
                                            {c.name} ({c.dial_code})
                                        </MenuItem>
                                    ))}
                                </TextField>
        ...
        {
            getField: data => <input type="hidden" name="role" value={data.role ?? 'interviewer'} />,
        },
    ], [fieldPadding, firstFieldPadding, formSubtitle, formTitle, inputSx, isEditing, navigate, passwordError, showPassword, theme.palette.background.default, theme.palette.divider, theme.palette.text.primary]);

    const initialValuesDict = useMemo(
        () => ({
            ...initial,
            role: initial.role ?? 'interviewer',
    ...
    const handleSubmitExtended = (_e, apiData) => {
        if (apiData && (apiData._id || apiData.success)) {
            setAlertConfig({
                open: true,
                message: initial._id ? 'Interviewer updated successfully' : 'Interviewer added successfully',
                severity: 'success',
            });
    ...
    return (
        ...
        >
            <MUIAlert open={alertConfig.open} message={alertConfig.message} severity={alertConfig.severity} onClose={handleAlertClose} />
            <MUICreateForm
                fields={fields}
                initialValuesDict={initialValuesDict}
                submitUrl={initial._id ? `/api/users/${initial._id}` : '/api/users'}
            ...
            >
                ...
                >
                    <MUIButton onClick={() => navigate('/interviewers')} sx={{ px: 2 }}>
                        Cancel
                    </MUIButton>
                    <MUIButton
                        type="submit"
