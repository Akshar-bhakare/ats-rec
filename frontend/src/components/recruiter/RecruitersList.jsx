...
export default function RecruitersList() {
    ...
    const loadRecruiters = useCallback(async () => {
        ...
        try {
            ...
            setRows(
                list
                    .filter(u => u.role === 'recruiter' && !u.isArchived)
                    .map(u => ({ ...u, id: u._id }))
            );
        } finally {
    ...
    const renderNameCell = (row) => {
        ...
        return (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.2, height: '100%' }}>
                ...
                {renderPrimarySecondaryCell(fullName, 'Recruiter')}
            </Box>
        );
    };
            ...
            <Box sx={{ m: { xs: 0, sm: 2, md: 3 }, p: { xs: 1.5, sm: 2 }, mt: 0, pt: 2 }}>
                {loading && filteredRows.length === 0 ? (
                    ...
                    <MUIRetrieveDataGrid
                        ...
                        rows={filteredRows}
                        columns={columns}
                        onRowClick={row => navigate(`/recruiters/${row.id}`)}
                        onEdit={row => {
                            setCurrent(row);
                        ...
                        createButton={
                            <TableFilterBar
                                searchLabel="Search Recruiters"
                                searchValue={searchQuery}
                                onSearchChange={setSearchQuery}
                                filterTitle="Filter Recruiters"
