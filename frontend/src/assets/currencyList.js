...
/** Common currency codes shown at the top of the dropdown (in display order) */
const COMMON_CURRENCY_ORDER = [
    'AED', 'USD', 'INR', 'EUR', 'GBP', 'SGD', 'CAD', 'AUD', 'JPY', 'CHF', 'CNY', 'SAR', 'HKD', 'MYR', 'BDT', 'PKR', 'LKR', 'NPR',
];

const currencyMap = Object.fromEntries(currencyList.map(c => [c.code, c]));

const toOption = (c, group) => ({
    label: `${c.code} - ${c.name} (${c.symbol})`,
    value: c.code,
    group,
});

/** Options formatted for Autocomplete with grouping: Common currencies first, then all A-Z */
export const currencyOptions = [
    ...COMMON_CURRENCY_ORDER
        .filter(code => currencyMap[code])
        .map(code => toOption(currencyMap[code], 'Common')),
    ...currencyList
        .map(c => toOption(c, 'All Currencies')),
];
